...
没有 `nativeId` 的窗口 id 跟随标题：在列出与动作之间标题发生变化的窗口读作 `COMPUTER_WINDOW_GONE`。存在 `nativeId` 时，不透明 id 在标题变化后仍然有效，销毁后再创建的原生句柄会铸造新 id。在 `windowCacheMs` 内，上次枚举之后关闭的窗口仍会被列出；对它的下一次快照会由原生树报错。适配器的结构化模块接口是针对 simulang 大版本的维护义务；`loadSimulang` 拒绝未知 API 时降级到操作系统回退而不是崩溃。
