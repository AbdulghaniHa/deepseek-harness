...
Window ids without `nativeId` follow titles: a window whose title changes between listing and action reads as `COMPUTER_WINDOW_GONE`. When `nativeId` is present, opaque ids survive title changes and a destroyed then recreated native handle mints a new id. Within `windowCacheMs`, a window closed after the last enumeration still lists; the next snapshot on it fails from the native tree. The adapter's structural module interface is a maintenance obligation against simulang's major versions; `loadSimulang` rejecting an unknown API degrades to the OS fallback rather than crashing.
