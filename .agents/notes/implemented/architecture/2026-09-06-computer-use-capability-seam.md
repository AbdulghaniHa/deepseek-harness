...
2. `@deepseek-ai/dsh-computer-use-local` is the shipped provider id `local`. It spawns `lib/host.js` through `ctx.subprocess` and speaks newline-delimited JSON-RPC over stdio. The helper loads `@simular-ai/simulang-js` v13, declared as an optional dependency so `pnpm install` fetches the platform binary ([adapter note](../bug-fix/2026-09-07-simulang-v13-adapter-and-optional-dependency.md)), and otherwise uses an OS fallback (`osascript` / `screencapture` on macOS). A crash fails the in-flight call with `COMPUTER_HOST_CRASHED`; the next call starts a new helper.
