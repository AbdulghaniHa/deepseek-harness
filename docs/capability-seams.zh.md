...
flowchart LR
  ...
  svc_computer["ctx.computer<br/>Computer-use provider registry"]
  ...
  pkg_computer_use --> svc_computer
  pkg_computer_use_local --> svc_computer
...
| `ctx.browser` | `seam` | [`browser`](../packages/browser/browser) | [`browser-chrome-extension`](../packages/browser/browser-chrome-extension) | [`tool-browser`](../packages/browser/tool-browser) | - | Chrome Native Messaging 提供方注册到同一个 ctx.browser seam；tool-browser 负责稳定的面向模型 browser_* 名称。 |
| `ctx.computer` | `seam` | [`computer-use`](../packages/computer-use/computer-use) | [`computer-use-local`](../packages/computer-use/computer-use-local) | [`tool-computer-use`](../packages/computer-use/tool-computer-use) | - | 本地 helper 提供方注册到同一个 ctx.computer seam；tool-computer-use 负责稳定的面向模型 computer_* 名称。 |
